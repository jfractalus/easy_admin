...
(function ($) {
    ...
    var getCurrentSearchControls = function (that) {
        var searchControls = 'select, input, daterangepicker, datetimerangepicker';
        if (that.options.height) {
            searchControls = 'table select, table input, table daterangepicker, table datetimerangepicker';
        }
            ...
            if (header.find('.date-filter-control').length > 0) {
                $.each(that.columns, function (i, column) {
                    if (column.filterControl !== undefined){
                        var filterControlValue = column.filterControl.toLowerCase();
                        if(filterControlValue === 'datepicker') {
                        ...
                        }else if(filterControlValue === 'daterangepicker' || filterControlValue === 'datetimerangepicker') {
                            var containsTime = filterControlValue === 'datetimerangepicker';
                            header.find('.date-filter-control.bootstrap-table-filter-control-' + column.field)
                                .daterangepicker({
                                    ...
                                    timePicker: containsTime,
                                    timePicker24Hour: containsTime,
                                    ...
                                    if(filterControlValue === 'datetimerangepicker'){
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        filterTemplate: {
            ...
            datetimerangepicker: function (that, field, isVisible) {
                return sprintf('<input type="text" class="form-control date-filter-control bootstrap-table-filter-control-%s" style="width: 100%; visibility: %s">', field, isVisible);
            }
        },
        //internal variables
    ...
    var formatDate = function (date) {
        var containsTime = date.includes(":");
        if(containsTime) {
            return new Date(date.replace(/(\d{2}).(\d{2}).(\d{4}) (\d{2}):(\d{2})/, "$2/$1/$3 $4:$5"));
        }else{
            return new Date(date.replace(/(\d{2}).(\d{2}).(\d{4})/, "$2/$1/$3"));
        }
    };

    var isDateInRange = function (dateWithTime, range) {
        var returnValue = false;
        var dateFrom = new Date(Date.parse(formatDate(range[0]))),
            dateTo = new Date(Date.parse(formatDate(range[1]))),
            date = new Date(Date.parse(formatDate(dateWithTime)));
        if(dateFrom){
            returnValue = date.getTime() >= dateFrom.getTime();
            if(returnValue && dateTo){
                returnValue = date.getTime() <= dateTo.getTime();
            }
        }
        return returnValue;
    };
    ...
    BootstrapTable.prototype.initSearch = function () {
        ...
        this.data = fp ? $.grep(this.data, function (item, i) {
            for (var key in fp) {
                ...
                // Filter for daterange component
                // if one date in input, it'll be taken as 'fromDate'
                if(thisColumn && (thisColumn.filterControl === 'daterangepicker' || thisColumn.filterControl === 'datetimerangepicker')){
                    if(!isDateInRange(value, fval.split(" - "))){
                        return false;
                    }
                }else{
    ...
    BootstrapTable.prototype.onDaterangeColumnSearch = function (event) {
